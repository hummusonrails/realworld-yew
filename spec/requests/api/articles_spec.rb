...
RSpec.describe 'Api::Articles', type: :request do
  ...
  let(:new_article) { Article.new(id: 'new-article-id', title: 'New Article', description: 'New Description', body: 'New Body', tag_list: ['tag1', 'tag2'], author_id: current_user.id, slug: 'new-article') }
  ...
  let(:query_result) { instance_double(Couchbase::Cluster::QueryResult, rows: [article.to_hash.merge('_default' => article.to_hash)]) }
  ...
  before do
    ...
    allow(mock_cluster).to receive(:query).with("SELECT META().id, * FROM RealWorldRailsBucket.`_default`.`_default` WHERE `slug` = ? AND `author_id` = ? LIMIT 1", options).and_return(query_result)
  ...
  describe 'GET /api/articles/:slug' do
    it 'returns the article' do
      ...
      get '/api/articles/test-title', headers: headers
    ...
    it 'returns an error if the article is not found' do
      ...
      headers = { 'Content-Type': 'application/json' }
      get '/api/articles/unknown-title', headers: headers
  ...
  describe 'POST /api/articles' do
    it 'creates a new article' do
      ...
      post '/api/articles', params: { article: { title: 'New Article', description: 'New Description', body: 'New Body', tag_list: ['tag1', 'tag2'] } }.to_json, headers: headers
  ...
  describe 'PUT /api/articles/:slug' do
    ...
    it 'updates the article' do
      ...
      allow(mock_cluster).to receive(:query).with("SELECT META().id, * FROM RealWorldRailsBucket.`_default`.`_default` WHERE `slug` = ? AND `author_id` = ? LIMIT 1", anything).and_return(query_result)
      ...
      put '/api/articles/test-title', params: { article: updated_attributes }.to_json, headers: headers
  ...
  describe 'DELETE /api/articles/:slug' do
    it 'deletes the article' do
      ...
      delete '/api/articles/test-title', headers: headers
  ...
  describe 'POST /api/articles/:slug/favorite' do
    it 'favorites the article' do
      ...
      post '/api/articles/test-title/favorite', headers: headers
  ...
  describe 'DELETE /api/articles/:slug/unfavorite' do
    it 'unfavorites the article' do
      ...
      delete '/api/articles/test-title/unfavorite', headers: headers
  ...
  describe 'GET /api/articles/feed' do
    it 'returns the user feed' do
      ...
      get '/api/articles/feed', headers: headers
      expect(response).to have_http_status(:ok)
      expect(JSON.parse(response.body)['articles'].first['title']).to eq('Test Title')
    end
  end
